        ...
        while line:
            ...
            if "<http://ma-graph.org/property/paperCount>" in line:
                ...
                if id_pc == id:
                    ...
                    #index = bisect.bisect_left(papercounts, int(papercount))
                    #fieldofstudy_ids.insert(index, id)
                    #fieldofstudy_names.insert(index, name)
                    #fieldofstudy_names_utf8.insert(index, name_utf8)
                    #papercounts.insert(index, int(papercount))
                    #if len(fieldofstudy_ids) == 101:
                    #    fieldofstudy_ids = fieldofstudy_ids[1:]
                    #    fieldofstudy_names = fieldofstudy_names[1:]
                    #    fieldofstudy_names_utf8 = fieldofstudy_names_utf8[1:]
...
def get_linkings_oc_makg(csvfile_p, outfile_p, notfoundfile_p, operation, makg_doi_data, lengths):
    ...
    with open(csvfile_p, 'r', encoding='utf8') as csvfile, open(notfoundfile_p, operation) as notfoundfile, open(outfile_p, operation, newline='\n') as outfile:
        ...
        for row in csv_reader:
            ...
            try:
                oci, citing_doi, cited_doi, _, _, _, _ = row
            except ValueError:
                continue

            rows_in_oc += 1

            l_citing_doi = citing_doi.lower()

            if len(l_citing_doi) in lengths:

                makg_indices = find_index(makg_doi_data[len(l_citing_doi)], dict_list_index_get_member(makg_doi_data[len(l_citing_doi)], 'udoi'), l_citing_doi) #TODO

                if makg_indices[0]:
                    mappings_oc_makg += 1
                    for i in makg_indices[1]:
                        if makg_doi_data[len(citing_doi)][i]['occ'][0] == 0:
                            writer.writerow({'DOI': citing_doi, 'MAGPaperID': makg_doi_data[len(l_citing_doi)][i]['id']})
                            makg_doi_data[len(l_citing_doi)][i]['occ'][0] += 1
                            linkings_oc_makg += 1
                else:
                    notfoundfile.write('{}\n'.format(citing_doi))

    return rows_in_oc, linkings_oc_makg, mappings_oc_makg
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 2:  # /slow/users/mfa/makg/ mag-2019-12-26
        homedir = sys.argv[1]
